...
export async function sendDMResponse(clientManager: ClientManager) {
    clientManager.getClient().on('message', async message => {
        ...
        if(
            dismissRestrictedCommands(messageContent, [RESTRICTED_QUAN_DM_COMMANDS])
            || dismissRestrictedPrefixes(messageContent, [RESTRICTED_QUAN_DM_PREFIXES])
        ) return;
...
!reboot: reboots Bottie. If this doesn't fix bugs, let Victor know
`;
